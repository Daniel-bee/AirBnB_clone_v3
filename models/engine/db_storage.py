...
class DBStorage:
    ...
    def getCity(self, ids):
        """Get city by state id"""
        data = self.__session.query(City).filter(City.state_id == ids).all()
        if data:
            return data
        return None

    def getPlace(self, ids):
        """Get city by state id"""
        data = self.__session.query(Place).filter(Place.city_id == ids).all()
        if data:
            return data
        return None
